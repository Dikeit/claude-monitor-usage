...
function parseCredentialObject(data) {
...
class CredentialProvider {
     ...
     */
    async getToken(autoDetect = true) {
    ...
    async promptForToken() {
        const accessToken = await vscode.window.showInputBox({
            ...
            prompt: 'Paste your Claude OAuth access token. ' +
     ...
     */
    async tryWindowsCredentialManager() {
...
`;
        try {
            const { stdout } = await execFileAsync('powershell', ['-NoProfile', '-NonInteractive', '-Command', psScript], { timeout: 10000 });
