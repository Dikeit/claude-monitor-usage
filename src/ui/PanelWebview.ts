...
import { HistoryPoint } from '../storage/HistoryStore';

export class PanelWebview implements vscode.Disposable {
  private panel: vscode.WebviewPanel | undefined;
  private lastSnapshot: UsageSnapshot | undefined;
  private lastHistory: HistoryPoint[] = [];
  ...
  show(): void {
    ...
    this.panel.webview.onDidReceiveMessage((msg: { command: string }) => {
      if (msg.command === 'refresh') { this.onRefreshRequest(); }
    });

    this.renderHtml();
  }

  update(snapshot: UsageSnapshot, history: HistoryPoint[]): void {
    this.lastSnapshot = snapshot;
    this.lastHistory = history;
    if (this.panel) { this.renderHtml(); }
  }
  ...
  isVisible(): boolean { return this.panel?.visible ?? false; }
  ...
  private buildLoadingHtml(nonce: string): string {
...
<body class="loading-body">
  ...
  private buildHtml(snap: UsageSnapshot, history: HistoryPoint[], nonce: string): string {
...
function barRow(label: string, pct: number, resetStr: string, tokenStr: string): string {
...
function buildTrendSvg(history: HistoryPoint[]): string {
...
function tokenLine(used?: number, limit?: number): string {
